...
- **Import from LuckPerms** — a server moving off LuckPerms had to retype everything. It can now bring the groups, the players and their nodes over, in two steps that are never one: reading changes nothing and answers with a report, applying writes that report and nothing else, after a timestamped copy of every config file. A group becomes a grade with its weight, its parents and the groups it refuses; a node set to false becomes a DENY; a player keeps their groups, their refusals and their own nodes; `minecraft.command.<x>` becomes `customperm.command.<x>` and exposes `<x>`, without which the node would grant nothing. What has no equivalent is named in the report rather than dropped in silence: contexts other than a single world, meta and display names, tracks, and the nodes other mods read. Users are read through a handful of storage searches rather than one load per account, and on players only the families CustomPerm can read are looked at, which the report says. Run it with `/customperm import preview` then `/customperm import confirm`, or from the new Import page, both asking for `customperm.manage.grades`, `customperm.manage.commands` and `customperm.manage.luckperms` together. A preview older than ten minutes is read again rather than trusted, and applying it forgets it, so a second confirm cannot import twice. While LuckPerms is installed it still decides permissions: what is imported waits, readable on the Grades page, and takes over the day LuckPerms is removed.
