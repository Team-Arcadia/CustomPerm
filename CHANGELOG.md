...
- **Cluster mode** — several servers without LuckPerms had no way to share their configuration: each kept its own files, and a grade made on the hub had to be made again on every server. With `"cluster": { "enabled": true }` in `settings.json` and [Arcadia Lib](https://www.curseforge.com/minecraft/mc-mods/arcadia-lib) 1.3.0 or later connected to one MySQL or MariaDB database, the servers share their grades and what players hold, exposed commands, aliases, rate-limit rules and the activity log, each part switchable; rate-limit counters too when asked. Off by default, read at start; nothing changes for a server that does not turn it on, and nothing is done with LuckPerms, which shares its own storage.
  ...
  - Arcadia Lib is optional: CustomPerm ships no database driver and loads without it.
