...
- **License text moved to the kit's 2.1** — the proprietary terms are unchanged in what they permit and forbid, but the text is restructured: the five enforceability clauses become `10.1` to `10.5` under a single `10. GENERAL`, and `PERMISSIONS AND CONTACT` becomes `11`. Sections 1 to 9 keep their numbering, so every reference to `§2.2`, `§3(b)`, `§5.1`, `§5.2` or `§5.3` still resolves. The revocation clause regains the sentence stating that withdrawing the modpack permission needs separate written notice to that maintainer and does not apply retroactively. The text carries no third-party clause and no carve-out of any kind: the only code in CustomPerm not covered by these terms is the portions contributed under MIT before the relicensing, which `NOTICE.md` records. No published build is governed by these terms: 1.1.0 will be the first.

- **Cluster mode's direct connection needs a database driver from another mod (breaking for cluster users)** — CustomPerm used to pack MariaDB Connector/J inside its own jar. It ships no driver at all now, and `cluster/JdbcDrivers` instantiates by name whichever JDBC driver the server already carries, MariaDB's first then MySQL's, building the URL that driver accepts and naming the TLS mode in its own vocabulary, since the two reject each other's. A server carrying none does not start cluster mode and says so through the new `NO_JDBC_DRIVER` state rather than failing obscurely; installing Arcadia Lib, or any mod that brings a driver, restores it. The other modes are untouched: the mod still runs alone, without LuckPerms and without a database.

- **Nothing third-party travels in the jar any more** — no nested jar, no `META-INF/licenses/`, no `credits` line about a bundled component. That is what lets the licence stay the kit's All Rights Reserved with sections 3 and 4 applying without exception, and it removes the corresponding-source duty that distributing an LGPL library placed on every release.

- **The README badge says source-available** — it read `All Rights Reserved` alone, which describes a closed project and is the first thing a visitor sees on a repository whose point is that the source is readable. It now reads `source-available (All Rights Reserved)` and still links to `LICENSE`.

- **`mod_license` names the licence and its identifier** — `gradle.properties` said `All Rights Reserved` alone while `LICENSE` declared `LicenseRef-CustomPerm-ARR`, the kind of gap issue #3 was about. The field now reads `All Rights Reserved (LicenseRef-CustomPerm-ARR)`: the prose stays readable in the in-game mod list, which is where a player meets it, and the identifier ties it to the licence file and to the SPDX headers.

- **Administering CustomPerm needs a granted permission, not just an op level (breaking)** — `/customperm` and the admin interface used to open for any level-2 operator. They now need op level 2 **and** an explicitly granted `customperm.admin`, and every change needs the `customperm.manage.<area>` node of its area (`customperm.manage.commands`, `.aliases`, `.ratelimits`, `.grades`, `.logs`, `.config`, `.luckperms`), for the text commands and the interface alike. The op level alone opens nothing, level 4 included, so a player made operator by mistake gets no access; the nodes alone open nothing either, so op level 2 is still required (audit retest R02). The console always has access, and the host of a singleplayer or LAN world stands in for it, that world having no console. Subcommands whose node is missing are hidden from tab-completion.
  - **After upgrading, nobody holds these nodes**: grant them from the console with `customperm grade create admins`, `customperm grade addperm admins customperm.*`, `customperm grade assign <name> admins`, or `lp user <name> permission set customperm.* true`. The server log says so at every start, and names each operator who logs in without the entry node.
  - The interface write nodes `customperm.gui.<area>.edit` are replaced by these `customperm.manage.<area>` nodes, shared with the commands; `customperm.gui.luckperms.edit` becomes `customperm.manage.luckperms`.
...
- **A third GameTest mode, with Arcadia Lib and nothing else** — `runGameTestServerArcadia` copies Arcadia Lib into `run/gametest-arcadia/mods` and runs the driver tests alone, in their own `customperm_driver` namespace. It is the only way to prove that `cluster/JdbcDrivers` can load a driver class another mod brought inside a nested jar, which is the production configuration and something no unit test can reach. Checked in both directions: green against the real Arcadia Lib, and failing with the driver named when the class to look for is made bogus. The mode runs those three tests alone on purpose: Arcadia Lib registers its network channels without `optional()` and sends one on every player join, which kills every test that joins a test player.

- **Cluster mode through Arcadia Lib asks something of the players** — both READMEs now say it. CustomPerm's own channel is optional, so a client without the mod joins and loses only the admin interface. Arcadia Lib's channels are required, so every player must install Arcadia Lib client-side to join a server running it, whether or not cluster mode is on. That is a property of Arcadia Lib; the documentation now states it rather than letting a server owner discover it when players are refused.

- **The empty case of the direct connection is covered** — `Cluster.directStore` takes its driver as an argument, so `DirectStoreTest` reaches the state a server with no driver produces without needing a server that carries none, and asserts that the message names a mod to install.

- **The build refuses a jar that no longer matches the licence** — the licence carries no third-party clause, and that only holds while the jar carries no third-party file. `./gradlew verifyPackage`, wired into `check` so `build` runs it, opens the produced jar and fails on a `META-INF/jarjar/` entry, a `META-INF/licenses/` entry, any file outside `com/arcadia/customperm/`, `data/customperm/`, `assets/customperm/` and a short allowlist, a missing or mis-declared `META-INF/LICENSE`, or mod metadata that no longer names `LicenseRef-CustomPerm-ARR`. The failure names the offending entry and says what it implies. Checked in both directions: green on the real jar, and failing with `nested third-party jar: META-INF/jarjar/fake-library.jar` when one is put back on purpose.

- **Aliases take arguments** — an alias was a command with no argument, so `/heal <player>` had to be written as one alias per player or worked around with `@p`. An alias now declares the arguments it takes, and its steps reach them with `${name}`: `say [WARNING] ${target}: ${reason}`. Four types, each validated and completed by Brigadier itself: `player` (an online player, resolved to their name, suggested as vanilla suggests them), `integer` (with an optional range), `word` (with optional declared choices, which are also what it suggests) and `text` (the rest of the line, only as the last argument). An argument can be made optional and given a default, substituted when it is left out. Declaring a required argument after an optional one, or anything after a `text` one, is refused rather than silently broken, those being Brigadier's own rules.
  - `${name}` was chosen because no command syntax produces a dollar sign followed by a brace: the NBT and JSON braces of a real step are never mistaken for an argument. A `${name}` the alias does not declare stays in the step as typed, reported as a warning rather than refused, since a step and an argument can be declared in either order.
  - **A value never carries an entity selector unless the argument allows it.** Steps run at op level 4, so `@a` reaching one would act on everyone instead of the player named. A `word` cannot hold an `@` at all, a `player` is resolved before substitution, and a `text` refuses one until `customperm alias param selectors <alias> <name> true`.
  - `aliases.json` gains a separate `aliasParameters` map, so a file written before this release loads unchanged and an alias with no argument still reads as a plain list of steps. In cluster mode an alias row carries its arguments alongside its steps, and a server still on an older version reads the steps and ignores the rest.
  - `/customperm alias param add|remove|move|optional|default|range|choices|selectors` and `/customperm alias params`, plus an Args tab on the Aliases page of the interface.
...
- **Cluster mode** — several servers without LuckPerms had no way to share their configuration: each kept its own files, and a grade made on the hub had to be made again on every server. With `"cluster": { "enabled": true }` in `settings.json` and one MySQL or MariaDB database, reached either directly (`"connection": "direct"`, the database and a server name in `settings.json`) or through [Arcadia Lib](https://www.curseforge.com/minecraft/mc-mods/arcadia-lib) 1.3.0 or later (the default), the servers share their grades and what players hold, exposed commands, aliases, rate-limit rules and the activity log, each part switchable. Rate-limit counters are shared rule by rule: `/customperm ratelimit scope <command> server|network|hub,survival`, or the Rate limits page; `server`, each server counting its own, is the default. Off by default, read at start; nothing changes for a server that does not turn it on, and nothing is done with LuckPerms, which shares its own storage.
  ...
  - The others apply what changed every `pollSeconds` (2 by default). Permission checks read memory as before: no database access on that path. At rest each server sends one query per poll for all shared parts, one for the log and one for shared uses, plus a heartbeat every 10 s, and never reads a row twice (measured: about 2 queries/s and under 1 KB/s per server).
  - The first server fills the database from its files; a server joining a filled database takes it, its files copied to `backup/` first.
  - Database unreachable: the server keeps the rights it last read, refuses admin changes, alerts the admins, and catches up when it answers again.
  - The server name is Arcadia Lib's `server_id`; a second running server under the same name stays out and says why. That name makes `server=<name>` a context, and import and export carry LuckPerms' `server=` nodes while a cluster runs.
  - The dashboard shows the cluster; log entries from other servers carry `@<server>`.
  - Arcadia Lib is optional. The direct connection uses MariaDB Connector/J 3.5.10 (LGPL-2.1-or-later), packed unmodified inside the jar (+0.8 MB), with TLS as an option (`tls`: `off`, `trust`, `verify`). See NOTICE.md.

- **Permissions of other mods** — a node another mod reads was a dead string in a grade: nothing here answered the check. Mods that declare their nodes through NeoForge's permission API now get their answer from the grades, through a handler CustomPerm offers, `customperm:handler`, so `/customperm grade addperm vip somemod.feature` works for them like for a CustomPerm node, wildcards included, and completion lists the declared nodes. A yes/no node answers `true` for an explicit ALLOW, `false` for a DENY, and otherwise the default its mod gave it, never a refusal; a number or text node always answers its default; an offline player is resolved from the grades too. NeoForge keeps exactly one handler, the one `permissionHandler` names in `neoforge-server.toml`, and LuckPerms takes that value when it is still the default; so CustomPerm selects itself only without LuckPerms, when the value is still the default and the new `answerOtherMods` setting (`settings.json`, on by default) allows, never replaces a value an admin chose, and makes the change in memory only, so turning the setting off takes effect at the next start. The start log says which handler answers and why. The import now carries the nodes mods declared, on groups and players, searching players by each declaring namespace; nodes read without being declared are still left behind.

- **Tracks** — a server coming from LuckPerms promotes staff along ladders and had to assign and unassign grades by hand here. A track is now an ordered list of grades, lowest first, kept as a `tracks` map in `grades.json` beside the grades it names, so deleting a grade takes it off every track in the same write. `/customperm track create|delete|append|insert|remove|list` builds them, and `/customperm track promote|demote <player> <track>` moves a player one rung, following LuckPerms: a player on no rung is promoted onto the first, demoting from the first rung takes them off the track, the top and nowhere change nothing, and a player on two rungs of one track is refused rather than guessed at. Only grades held everywhere count as rungs; the rung given up takes its expiry with it and the one given is permanent, in one write, through the lockout guard, with the command tree sent again. It needs `customperm.manage.grades` and is refused while LuckPerms decides. The Players page gains a Tracks tab showing the player's rung on each ladder, with Promote and Demote. The import carries LuckPerms tracks with their groups in order, keeping a track that already exists here unless replacing, and the export writes them back after the players; tracks are no longer counted among what has no equivalent.

- **Per-world entries** — a server that wanted `/home` everywhere but in the Nether, or a builder grade in the creative world only, had no way to say it: every entry applied in every dimension. A node on a grade or a player, and a grade a player holds, can now be limited to one world with `world=<dimension>` after the node on `/customperm grade addperm|adddeny|removeperm|removedeny` and `/customperm user addperm|adddeny|removeperm|removedeny`, and after the grade on `/customperm grade assign|unassign` (`world=the_nether`, `world=mymod:mining`; a value without a namespace means `minecraft:`, as LuckPerms writes it). Tab-completion offers the loaded worlds. At the same specificity and from the same holder, an entry limited to the player's world outranks the same entry without one, like a contextual node in LuckPerms; the holder still comes first, so a heavier grade or the player's own node decides over a lighter grade's world node. The command tree is built per player, not per world, so a player gets theirs again on a dimension change: always with LuckPerms, whose nodes may be per world, and on the internal backend only while some entry is limited to one. The file adds `contexts` on a grade and `userContexts` at the top, keyed by a context written as `key=value` pairs, so the `server=<name>` a cluster mode would add needs no new shape; a key this version does not read is kept and matches nothing. A grade without one pays an empty-map check: the resolver benchmark, run before and after, shows no change beyond its error margin, and gains a case for a check made from a world and one a world node decides. An entry limited to a world is permanent. `/customperm user list` and the Grades and Players pages (a world box beside the node and player fields) show and set it. The import carries LuckPerms nodes and a player's groups limited to a single world, and still leaves behind any other context and a temporary entry limited to a world; the export writes them back with LuckPerms' `world` context, and replacing now clears customperm nodes and parents limited to one world along with the global ones.

- **Meta, and the number and text permission nodes of other mods** — a mod declaring a number or text node through NeoForge (a home limit, a rank label) always got its default here, and an import left LuckPerms meta behind, since nothing stored meta. Grades and players now carry meta, key to value: `/customperm grade|user meta <holder> [set <key> <value> [duration] [context] | unset <key> [context]]`, and a **Meta** tab on the Grades and Players pages. The permission handler answers a number node with the meta of the same name read as a whole number, its default when there is none or it is no number, and a text node with the value, the way LuckPerms answers them. The value is decided like a node, off the chat path: the player's own, then the heaviest grade, then the nearest ancestor, one limited to a context before the same holder's global one, the value that sorts first between two left equal; a grade without parents allocates nothing. The file gains `meta` and `metaExpiries` on a grade and in each context, and `userMeta` and `userMetaExpiries` at the top; keys are lowercased. The import carries LuckPerms meta nodes, with their context and expiry, one value per key and context (the permanent one, then the one that sorts first), and searches players for them too; the export writes them back, adding keeping a value LuckPerms has for the key and replacing clearing only the keys the grade sets. The interface actions and page data gained meta, so client and server must run the same build.

- **Mod check** — a server running CustomPerm without LuckPerms had no way to know which of its mods call LuckPerms' own API instead of NeoForge's. Those checks get no answer from CustomPerm, so a node granted to such a mod in a grade looked right and did nothing. `/customperm modcheck` reads the compiled classes of every installed mod once, off the server thread, and lists the mods whose classes name LuckPerms' API, each marked "LuckPerms only" or "also uses NeoForge's permission API", with what that means with or without LuckPerms. LuckPerms and CustomPerm themselves are left out, a jar that cannot be read is named rather than hiding the others, and the answer is kept until the next start. Needs `customperm.admin` only: it reads, it changes nothing. Answering those mods directly was studied and not done: it would need a copy of LuckPerms' API, which cannot load next to the real LuckPerms. The README says what still works without LuckPerms: a command of such a mod is opened by exposing it or through an alias, a feature in game outside any command is left to the mod's config, its author or LuckPerms, and forbidding actions in a place is pointed to ArcadiaGuard's zone protection.

- **A permission per track** — moving a player along a track needed `customperm.manage.grades`, the same as assigning any grade, so a moderator could not be trusted with one ladder alone. `customperm.track.<track>`, on top of `customperm.admin`, now allows `/customperm track promote|demote` on that track only, and the same buttons on the Tracks tab of the Players page, which enables them per track for the viewer; `customperm.track.*` covers every track, and `customperm.manage.grades` keeps allowing all of them. Nothing else of the grades opens to it. Moving oneself with the track node alone is refused, since a rung up the ladder may be the grade that hands out `customperm.manage.grades`; the lockout guard applies to every move. Creating or deleting a track now resends the command tree, since whether promote and demote show depends on the tracks that exist.

- **Promote and demote within a world** — a track moved only the grades a player holds everywhere, so a ladder of builder grades for the creative world could not be climbed there alone. `/customperm track promote|demote <player> <track>` now takes a context after the track (`world=the_nether`, or any context an entry takes), and the Tracks tab of the Players page takes it in a world box: the rungs are then the grades held in that context only, and the move gives and takes grades there only, like LuckPerms' promote with a context. A grade held everywhere is not a rung there, and the result says when it already covers that context; a grade refused everywhere or in that context is not given; the rung given up takes its expiry there with it; leaving the track there leaves no empty entry in the file. The page shows each player's rung in the context typed. The move rules, the lockout guard and `customperm.manage.grades` are the same as for a move everywhere. The interface's promote and demote actions now carry the context, so this version's client and server go together.

- **Nicknames** — a player could only be shown under their account name. An admin now sets a nickname with `/customperm user nick <player> set <nickname>` (shown with no argument, taken off with `clear`) or from a box on the Players page, and a player sets their own with `/nick <nickname>` or `/nick clear` once granted `customperm.nick`; `&` codes in their own need `customperm.nick.color`, since `&k` or a staff colour is what makes a nickname misleading. The nickname replaces the name itself, the `{name}` of the name format, wherever the game shows it, and applies with decoration off too; a team colour still applies around it, and hovering the name still shows the real one. It works on both backends: a nickname is not a permission, so it is stored in `grades.json` as `userNicknames` and read whoever decides permissions, and neither imported nor exported. A nickname that reads as another player's name or nickname is refused, letters compared without case, codes, spaces or `_ - .`; one's own name is always accepted, even when the name cache still pairs it with an old account. At most 16 visible characters. `/nick` is its own root, since `/customperm` needs `customperm.admin`, and it is left out, with a line in the log, when another mod already registers one.

- **Grade display names** — a grade was shown by its name everywhere, so `vip_t2` read as `vip_t2` in every listing, and an import left LuckPerms display names behind. A grade can now carry a display name, set with `/customperm grade displayname <grade> set <text>`, cleared with `clear`, shown with no argument, and edited from a box in the header of the Grades page. The listings (`grade list`, `user list`, `track list`) show `Very Important (vip)`, the Grades page shows it in the list, which its search also matches, and in the header, the name staying beside it; the parents it lists and the Players page, for the grades a player holds or refuses and the rungs of a track, show the display name too. It is display only: commands, `grades.json`, parents, assignments and tracks keep naming the grade by its name, so nothing is resent to players when it changes. At most 48 characters, on one line; two grades may share one, and the result says so. The file adds `displayName` on a grade, absent for none. An import carries a group's display name, the one that applies everywhere and for good; adding keeps one a grade already has, replacing takes LuckPerms'. A temporary one, one limited to a context or one this side would refuse is left behind and named in the report. An export writes it; adding keeps the one LuckPerms already has, replacing writes the grade's, or none. The import report also stopped counting a group's weight as left behind under a note about display names: the weight was always carried, and the one kind still without an equivalent, regular expression permissions, is now named.

- **Game mode, several worlds and static contexts** — an entry could be limited to one world only, so "builders get `/fly` in creative" or "this prefix in the Nether and the End" could not be said. A context is now any set of pairs joined with a comma, after the node, grade or parent as `world=` was: `gamemode=creative`, several values of one key read as either one (`world=the_nether,world=the_end`), and any key a static context sets for the whole server, like LuckPerms' `static-contexts` (`/customperm contexts set region eu`, stored as `staticContexts` in `settings.json`). Two keys must both hold, and at the same specificity and holder an entry naming more keys outranks one naming fewer. A key nothing on this server sets is refused, since the entry would apply nowhere; `server=` stays reserved for cluster mode. `/customperm contexts <player>` shows what holds for a player. The command tree and names follow a change of game mode as they follow a change of world. Prefixes take any context quoted after `where` (`prefix vip where "gamemode=creative" add ...`), and the pages' world box takes the same text. The resolver still compares a context in place without allocating, now deciding each key on its run of values; the player's contexts are cached per world and game mode, and rebuilt when the static contexts change. The import carries LuckPerms nodes limited to a game mode, several dimensions or a static key set here, and the export writes them back.

- **Entries both limited to a world and temporary** — an entry had a world or a duration, never both: the commands refused `7d world=the_nether`, and an import left such LuckPerms nodes behind. A node, a grade held, a grade parent, a refusal and a prefix limited to a world can now be temporary too. The commands take a duration and a `world=` together, in either order (`/customperm grade assign Steve vip 7d world=the_nether`), and the prefix commands take `addtemp` after `in <world>`; the Grades and Players pages send both boxes. An expired entry in a world stops counting at once, as a global one does, and the sweep removes it, drops the world entry it emptied, and logs it with the world. `/customperm user list`, `/customperm grade parent list` and the pages show the time left of an entry limited to a world beside its world. The file gains expiry maps inside each world entry, the same shape as the global ones: `permissionExpiries`, `deniedPermissionExpiries` and `refusedExpiries`, plus `parentExpiries` on a grade's and `gradeExpiries` on a player's; a file without them reads as before, everything permanent. The resolver reads them only where it already looks at an entry in a world, so a grade without one pays nothing more. The import carries LuckPerms nodes limited to a single world with their expiry, and the export writes them back temporary. The Nodes tab of the Grades and Players pages no longer lists a parent or a refusal limited to a world as an ALLOW node, and the Players page lists a refusal limited to a world with the others. The interface entry for one limited to a world gained its time left, so client and server must run the same build.

- **Parents, refusals and prefixes limited to a world** — a grade could inherit, refuse or show a prefix only everywhere, so "builders inherit the creative grade in the creative world" or "a red prefix in the Nether" could not be said, and an import left such LuckPerms entries behind. `world=<dimension>` now works on `/customperm grade parent add|adddeny|remove|removedeny`, `/customperm user denygrade|undenygrade`, and `/customperm grade|user prefix|suffix <holder> in <world> add|addtemp|remove|clear` (a prefix's text follows, so the world is named without `world=` there); the Parents and Chat tabs have a world box and show the world on the row. A parent inherited in one world is followed there only, at the depth of a global parent, with its own chain; a grade refused in one world, by a grade or by the player, is refused there only; a prefix limited to the player's world shows before the same holder's global one at the same priority, and names are rebuilt on a world change. A cycle is refused whatever the world it would close in. The file gains `parents`, `refused`, `prefixes` and `suffixes` in a grade's `contexts`, and `refused`, `prefixes` and `suffixes` in `userContexts`. A grade without contextual parents pays one empty-map check per grade walked, the contextual part kept out of the chain walk so it stays small; the resolver benchmark could not settle the difference, the machine varying more between two runs of the same build than between the two builds. The import carries LuckPerms group parents, refusals and prefixes limited to a single world, and a player's refusal limited to one; the export writes them back with the `world` context. The interface actions for parents, refusals and prefixes gained a context argument, so client and server must run the same build.

- **Several prefixes per holder, with priorities, stacking and expiry** — a grade or a player carried one prefix and one suffix, so a LuckPerms import kept the one it showed first and left the rest behind, and a prefix could not last a set time. Each is now an entry with a priority and an optional expiry, as in LuckPerms: the highest priority a player reaches shows, and at equal priority the order is the one it was (their own, then the heaviest grade, then the nearest ancestor). `/customperm grade|user prefix|suffix <holder>` lists them; `add <priority> <text>`, `addtemp <priority> <duration> <text>`, `remove <priority>` and `clear` edit them, a holder carrying one per priority. The Chat tabs of the Grades and Players pages list them with priority and time left, add and remove them, and preview the line. `prefixStack` and `suffixStack` in `settings.json` show the highest only (the default) or several in a row, up to a limit, between spacers, like LuckPerms' meta formatting stacks; `/customperm names stack` and a switch on the Chat tab set the mode. The sweep removes a prefix that has run out. The file gains `prefixes`/`suffixes` on a grade and `userPrefixEntries`/`userSuffixEntries` at the top; a single `prefix` or `userPrefixes` written before is read in at priority 0, which shows what it showed. The import carries every prefix with its priority and expiry, the export writes each with its own priority instead of the grade weight, and adding keeps one LuckPerms already has at that priority. The interface actions for prefixes changed (`GRADE_CHAT_ADD`/`REMOVE`, `USER_CHAT_ADD`/`REMOVE`, `NAMES_STACK`), so client and server must run the same build.

- **Temporary grade parents** — a grade could inherit another only for good, so "vip also gets moderator for the event weekend" meant a note and a command to remember, and an import left LuckPerms' temporary group parents behind. `/customperm grade parent add` and `parent adddeny` now take an optional duration, as does the Parents tab of the Grades page, which shows the time left; adding a parent already there with a duration makes it temporary from now, and without one permanent. The chain walk follows a temporary parent while it lasts and stops following it, or stops refusing it, the moment it has run out, before anything sweeps; a parent that ran out on one path is still reached through another that inherits it for good. The sweep removes it and records the removal. The file gains `parentExpiries` and `deniedParentExpiries` on a grade, so a file written before reads as permanent, and a grade without parents keeps its path. The import brings a group's temporary parents over with their expiry, and the export writes them back temporary.

- **Temporary entries** — a donor rank for thirty days or a trial moderator for a week meant a note in a calendar and a command to remember. A node on a grade or a player, a grade a player holds and a grade a player refuses can now carry an expiry: a duration after the node on `/customperm grade addperm|adddeny` and `/customperm user addperm|adddeny`, and an optional duration argument on `/customperm grade assign` and `/customperm user denygrade` (`30d`, `2h`, `1d12h`, `1w`, ten years at most). A node never holds a space, so a line written before durations existed reads exactly as it did. On an entry already there, the duration replaces what it had, temporary from now or permanent without one, as with LuckPerms' `settemp` and `set`. The file keeps its collections and adds a map of epoch seconds beside each (`permissionExpiries` and `deniedPermissionExpiries` on a grade, `userPermissionExpiries`, `userDeniedPermissionExpiries`, `userGradeExpiries`, `userDeniedGradeExpiries` at the top), so a file written before reads as permanent. The resolver ignores an entry whose time has passed, and a less specific entry below it answers instead, so an expiry is right before anything sweeps it; the expiry is read only on an entry that already matches, and a server with no temporary entry pays an empty-map check. Once a second, a sweep removes what has run out, saves, sends the command tree again, since a client would otherwise keep offering a command that now refuses, and records each removal in the activity log under a new `expiry` source. `/customperm user list` and the Grades and Players pages show the time each temporary entry has left, and the pages take a duration beside the node and player fields. The import brings LuckPerms' temporary nodes, a player's temporary groups and temporary refusals over with their expiry, leaving behind only a group's temporary parents and temporary prefixes, which are set for good here; the export writes temporary entries back temporary and leaves out the ones already over. Grade parents themselves stay permanent.

- **Chat prefixes and suffixes** — nothing on NeoForge showed a rank in chat: not the grades, and not the prefixes LuckPerms stores either. A grade can now carry a prefix and a suffix, and a player their own, resolved by the ranking permissions already use with the specificity step dropped: the player's own above every grade, then the heaviest grade, then the nearest ancestor, refusals and the default grade applying as they do to a node; between equal weights the text that sorts first wins, so the order grades were assigned in never decides. The ranking is the resolver's own, made generic rather than copied, and a permission check allocates nothing more than before. With `decorateNames` on (off by default, another chat mod may already do it), each player's name carries them, from the grades or, with LuckPerms, from LuckPerms' own prefix and suffix. **The name is decorated, never the message**: a rewritten message travels as unsigned content the client marks as modified, and a system message instead loses reporting and the secure chat indicator, while the sender name is not signed and NeoForge builds it through its name event. Every message stays signed and reportable. The cost, stated: the name carries the prefix wherever the game shows it (chat, death and advancement messages, `/msg`, `/me`, the join message, the tab list), not above a player's head, and the `<Name>` frame stays vanilla's. `nameFormat` (`{prefix}{name}{suffix}`) sets the layout and must keep `{name}`. Text takes `&` colour codes and `&#RRGGBB`; a section sign is dropped, never read as a code, and a line break or more than 64 characters is refused. A change to a grade, a reload or a LuckPerms recalculation rebuilds names at once. Set them with `/customperm grade prefix|suffix <grade> [text]`, `/customperm user prefix|suffix <player> [text]` and `/customperm names [on|off|format <format>]`, or on the new Chat tabs of the Grades and Players pages, which preview the line players would read. The import carries LuckPerms' prefix and suffix over, the one LuckPerms shows first when a holder has several, and the export writes them back, at the grade weight as priority and a player's own above every grade's.

- **Export to LuckPerms** — a server that built its grades here and installs LuckPerms later, for chat prefixes say, had to retype them in `/lp` for them to keep deciding. It can now write them into LuckPerms, in the same two steps as the import: reading changes nothing and answers with a report, confirming writes that report and nothing else. A grade becomes a group with its weight, its parents and the groups it refuses; a denied node becomes a node set to false; a player keeps their grades, their refusals and their own nodes; the default grade becomes a parent of the LuckPerms `default` group. Nothing is translated, CustomPerm on the LuckPerms backend reading `customperm.*` as it is. A grade name LuckPerms would refuse or lowercase is left out and named, never renamed quietly, and so is every entry naming it. Adding keeps what LuckPerms already holds, weight included, and where it sets a node the other way its value is kept and counted; replacing first clears the customperm nodes and parents of each group and player written, and their prefix or suffix only where the grade sets one, never their meta or the nodes of other mods. Each holder is one load and one save, groups before players, on a worker thread with progress, never the server thread; one export runs at a time, and a failure names the holder it stopped at, everything before it written and nothing after it. LuckPerms' storage cannot be copied from here, so the report, the page and the confirmation all say to run `/lp export` first, and the Export button stays disabled until the admin says it was done. An export that would take away the admin's own `customperm.admin` or `customperm.manage.grades` is refused before anything is written. Run it with `/customperm export preview` then `/customperm export confirm [replace]`, or from the To LuckPerms tab of the Import page, both asking for `customperm.manage.grades` and `customperm.manage.luckperms` together. `grades.json` is not changed.

- **Import from LuckPerms** — a server moving off LuckPerms had to retype everything. It can now bring the groups, the players and their nodes over, in two steps that are never one: reading changes nothing and answers with a report, applying writes that report and nothing else, after a timestamped copy of every config file. A group becomes a grade with its weight, its parents and the groups it refuses; a node set to false becomes a DENY; a player keeps their groups, their refusals and their own nodes; `minecraft.command.<x>` becomes `customperm.command.<x>` and exposes `<x>`, without which the node would grant nothing. What has no equivalent is named in the report rather than dropped in silence: contexts other than a single world, meta and display names, and the nodes other mods read. Users are read through a handful of storage searches rather than one load per account, and on players only the families CustomPerm can read are looked at, which the report says. Run it with `/customperm import preview` then `/customperm import confirm`, or from the new Import page, both asking for `customperm.manage.grades`, `customperm.manage.commands` and `customperm.manage.luckperms` together. A preview older than ten minutes is read again rather than trusted, and applying it forgets it, so a second confirm cannot import twice. While LuckPerms is installed it still decides permissions: what is imported waits, readable on the Grades page, and takes over the day LuckPerms is removed.

- **Refused grades** — once something inherited a grade, nothing could take it back out for one grade or one player. A grade now carries `deniedParents` and a player `userDeniedGrades`: a refused grade is not reached, whichever path would have led to it, the default grade included, which is how one player is exempted from the default grade. A refusal removes the grade from the resolution; it never turns what that grade allows into a denial. It is read where it is declared, so a grade nearer to the holder decides before a farther one, and a refusal cannot undo an inheritance a nearer grade asked for. Manage it with `/customperm grade parent adddeny|removedeny`, `/customperm user denygrade|undenygrade`, or the Grades page, where the Parents and Players tabs now carry the refusals beside the inheritances and the assignments. Saying a thing and its opposite is answered rather than stored: refusing a direct parent, inheriting a refused grade and assigning a grade the player refuses each point at the change to make first. Both fields are optional and empty when absent, so a `grades.json` written before this version resolves exactly as it did.

- **Grade inheritance** — sharing nodes between grades meant repeating them, and a grade had no way to build on another. A grade now carries `parents`: what a parent says applies where the grade says nothing as precise about the node, nearest ancestor first, so a grade overrides what it inherits while a more specific ancestor entry still wins. A chain answers with one verdict, offered at the weight of the grade the player actually holds, so a heavy parent does not smuggle its weight into a light grade. Manage it with `/customperm grade parent add|remove|list` or the Parents tab of the Grades page. A cycle is refused when it is created, naming the shortest path that already exists, rather than left to the resolver, which stops one silently. A grade with no parent keeps the previous resolution path, allocation free, and `parents` is optional and empty when absent, so a `grades.json` written before this version resolves exactly as it did.

- **Nodes carried by one player** — a permission could only be granted through a grade, so a single exception meant inventing a grade for one player. A node can now be carried by the player themselves: `userPermissions` and `userDeniedPermissions` in `grades.json`, `/customperm user addperm|removeperm|adddeny|removedeny <player> <node>`, `/customperm user list <player>`, and a **Players** page in the interface listing everyone who holds something of their own plus everyone online. Such a node outranks every grade the player holds at the same specificity, whatever a grade weighs, but it does not beat a more specific grade node: the most specific entry still decides first. Between a player's own ALLOW and DENY on the same node, the DENY wins. Writing needs `customperm.manage.grades`, like the grades themselves, and the lockout guard covers it: denying yourself `customperm.manage.grades` is refused and undone. The two new maps are optional and read as empty when absent, so a `grades.json` written before this version is unchanged.

- **Grade weight** — two grades held by the same player that covered a node just as specifically always resolved to DENY, whatever the intent, since a DENY won every tie. A grade now carries a `weight`, like a LuckPerms group weight: at equal specificity the heaviest grade decides, and a DENY still wins between equal weights. A weight never beats a more specific node, so it cannot be used to work around one, and it does not cross layers: the default grade is still read only when the player's own grades say nothing about the node. Set it with `/customperm grade weight <grade> <weight>`, negative allowed, or from the header of the Grades page, which now lists grades heaviest first. The field is optional and reads as 0 when absent, so a `grades.json` written before this version resolves exactly as it did.
...
- **Activity log** — a record of who changed what, and optionally of what players run.
  ...
  - **Logs** page in the interface (search, full entry, recording and masking switches gated by `customperm.manage.logs`) and `/customperm log admin|players [count]` in chat. Reading needs the same access as `/customperm`.
...
- **Read-only pages and the Import page, checked on a dedicated server** — without `customperm.manage.grades`, the Grades, Players and Aliases pages drew their read-only note over the text above it; it now comes first, and a selected grade or player says "Read-only:" in its title rather than at the end of a line a narrow panel cuts. `/customperm gui import` without the LuckPerms mod did nothing and said nothing; it now says why. On the Import page the explanation was cut mid-sentence and each report line at the panel edge; the explanation now takes the height it needs and report lines wrap.

- **Admin pages at the usual window sizes** — at the automatic GUI scale the right panel is about 180 to 260 pixels wide on common screens, and several rows assumed more. The Grades header now puts the display name and weight boxes on a second row when the title would be cut to a few letters; tab and button rows tighten their padding before clipping a label; the node, parent, player, chat and meta boxes share the row equally rather than leaving the first one empty (the meta key box had disappeared); the chat preview no longer draws over the boxes under it; the names settings (Highest or Stacked, Names decorated or plain) sit beside the preview they change; list rows keep the node, grade or key readable before the world and time left; tab counts include entries limited to a world; Promote and Demote follow the world typed on the Tracks tab; and a long value set by the page shows from its start.
