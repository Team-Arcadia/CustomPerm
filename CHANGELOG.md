...
- **License text moved to 2.1, and it now covers the components the jar carries** — the proprietary terms are unchanged in what they permit and forbid, but the text is restructured: the five enforceability clauses become `10.1` to `10.5` under a single `10. GENERAL`, and `PERMISSIONS AND CONTACT` becomes `11`. Sections 1 to 9 keep their numbering, so every reference to `§2.2`, `§3(b)`, `§5.1`, `§5.2` or `§5.3` still resolves. A new `12. THIRD-PARTY COMPONENTS` states that a component bundled in the jar is governed by its own license, that nothing in `§3` or `§4` restricts the rights that license grants, and, for a component under the LGPL, that you may modify it, modify CustomPerm as far as relinking it with your build requires, run the result for your own use, and reverse-engineer what debugging those changes needs. `§10.3` is made subject to it and incorporates `NOTICE.md` by reference, so the third-party notice is no longer superseded by the entire-agreement clause. `§12` survives termination. The revocation clause regains the sentence stating that withdrawing the modpack permission needs separate written notice to that maintainer and does not apply retroactively. No published build is governed by these terms: 1.1.0 will be the first.

- **The corresponding source of the bundled driver is offered from the same place as the build** — LGPL-2.1 section 4 and section 6(d) require the source of a distributed library to be reachable from the place the object code is offered, and a link to MariaDB's own repository is not that place. `NOTICE.md` now states, in both languages, that `mariadb-java-client-3.5.10-sources.jar` is attached to the release on CurseForge, on Modrinth and on GitHub Releases, plus a written offer valid at least three years from the last distribution as a fallback. It also stops claiming the component can be swapped by installing another build beside it: the bundled-jar loader pins version 3.5.10, so a different build is substituted inside the jar, which is what `§12` permits.

- **The mod list credits the bundled driver** — `mod_credits` adds a `credits` line to `neoforge.mods.toml` naming MariaDB Connector/J 3.5.10, its copyright holder, its licence, and where the licence text and the corresponding source are. LGPL-2.1 section 6 asks for a prominent notice with each copy of the work, and for the library's copyright notice to appear wherever the work displays copyright notices at runtime, which for a mod is the mod list.
...
- **`mod_license` declares the SPDX identifier** — `gradle.properties` said `All Rights Reserved` while `LICENSE` declared `LicenseRef-CustomPerm-ARR`; the jar metadata and the license file now say the same thing. This is the kind of gap issue #3 was about.
...
- **The French half of `NOTICE.md` denied what the English half stated** — it listed neither Arcadia Lib nor the bundled MariaDB Connector/J, and closed on "Aucun code tiers n'est empaqueté dans le jar CustomPerm", the opposite of the English text. Since `§10.5` makes the English text authoritative, a French reader had no notice at all for an LGPL component shipped inside the jar. Both halves now carry the same table, the same description of the nested jar and where its license text and source are, and the same pointer to `§12`.
