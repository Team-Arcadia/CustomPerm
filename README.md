...
> **Where the builds are.** Published builds are distributed on [CurseForge](https://www.curseforge.com/minecraft/mc-mods/customperm) and [Modrinth](https://modrinth.com/mod/customperm). The current version is **1.1.0**.
>
...
| Import | Only when LuckPerms is installed: brings its groups, players and nodes over, in two steps. Read LuckPerms answers with the report and changes nothing, Import applies that report and nothing else, after a backup. **Choose** picks the groups, players, tracks and kinds of entries carried, as ticks, and the report follows it; adding to grades of the same name or replacing them, which then empties only the kinds chosen. Needs the three write nodes together. A second tab, **To LuckPerms**, exports the grades the other way: Read the grades, **Choose** what goes, then Export, which stays disabled until the admin says LuckPerms is backed up; the page follows the progress while it runs. Needs `customperm.manage.grades` and `customperm.manage.luckperms` |
...
**Completion.** Every field that names something existing proposes it while you type: permission nodes, grades, players who joined before, contexts (`world=`, `gamemode=`, the static ones and `server=` for each cluster member), durations, commands and aliases, the first word of an alias step then its `${arguments}` and player names, meta keys and the values already used for the key typed, prefix and suffix texts in use, and the names listed on the page in its search box. The rest of the best candidate shows after the cursor and a list opens under the field: **Tab** takes the highlighted candidate, **Up** and **Down** move through the list (Down also opens it on an empty field), **Enter** submits what is typed unless you moved through the list, **Escape** closes it, and a click takes a candidate. Matching ignores case and also finds a word inside a node (`fly` finds `essentials.fly`). Fields that create a name (new grade, new alias, new argument, display name, nickname) and number fields propose nothing. The server sends this vocabulary with the first page and again only when it changed.
...
| `/customperm import select` | Shows what the next import carries. |
| `/customperm import select <groups\|players\|tracks\|kinds> <set\|add\|remove> <all\|none\|names>` | Chooses what the next import carries: the groups, the players (by name or UUID), the tracks, and the kinds of entries (`nodes`, `parents`, `chat`, `meta`, `contexts`, `temporary`, `commands`). Names are separated by spaces or commas and completed from what was read; an unknown one refuses the whole change. The next `preview` and `confirm` follow the selection, which is kept across reads. |
| `/customperm import select reset` | Carries everything again. |

**Choosing what is imported.** Leaving a group, a player or a kind out keeps it out of the import, and replacing then
empties only the kinds chosen, entries limited to a context only when `contexts` is chosen: what is not chosen stays as
it was. A group left out that a chosen group inherits, or that a chosen player holds, is kept as a reference when a
grade of that name exists already here, and dropped otherwise, the report naming each one. Only the commands the
imported nodes need are exposed. On the Import page, **Choose** shows the same thing as ticks: Groups, Players (with a
search), Tracks and Kinds, with All and None. A group always comes with its weight and display name.
...
| `/customperm export select ...` | The same choice for an export: grades, players, tracks and kinds (`commands` does not apply), `reset` to carry everything. Replacing clears only the kinds chosen, and a reference to a grade left out is kept when LuckPerms has that group already. |
...
- In the interface, the same choice is a row of toggles: **All**, then every member heard, this server marked with
  a house, and a warning mark on a listed name no member answers to right now. A note under them says when this
  server keeps that part local or has no cluster name.
